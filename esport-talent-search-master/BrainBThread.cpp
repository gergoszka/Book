...
BrainBThread::BrainBThread ( int w, int h )
{
        ...
        Hero other1 ( this->w / 2 + 200.0 * std::rand() / ( RAND_MAX + 1.0 ) - 100,
                      this->h / 2 + 200.0 * std::rand() / ( RAND_MAX + 1.0 ) - 100, 255.0 * std::rand() / ( RAND_MAX + 1.0 ), 5, "Caleb Entropy" );
        Hero other2 ( this->w / 2 + 200.0 * std::rand() / ( RAND_MAX + 1.0 ) - 100,
                      this->h / 2 + 200.0 * std::rand() / ( RAND_MAX + 1.0 ) - 100, 255.0 * std::rand() / ( RAND_MAX + 1.0 ), 3, "Jester Entropy" );
        Hero other4 ( this->w / 2 + 200.0 * std::rand() / ( RAND_MAX + 1.0 ) - 100,
                      this->h / 2 + 200.0 * std::rand() / ( RAND_MAX + 1.0 ) - 100, 255.0 * std::rand() / ( RAND_MAX + 1.0 ), 5, "Nott Entropy" );
        Hero other5 ( this->w / 2 + 200.0 * std::rand() / ( RAND_MAX + 1.0 ) - 100,
                      this->h / 2 + 200.0 * std::rand() / ( RAND_MAX + 1.0 ) - 100, 255.0 * std::rand() / ( RAND_MAX + 1.0 ), 7, "Fjord Entropy" );
        QTimer *timer = new QTimer(this);

        connect(timer, &QTimer::timeout,[=](  ) { this->randomMove(); });
...
void BrainBThread::randomMove(){
    for ( Hero & hero : heroes ){
        QDesktopWidget dw;
        if(hero.name=="Glados Entropy"){
            hero.randxy(dw.width(), dw.height(), ( h<w ) ?h/2:w/2);
        }
    }

}
void BrainBThread::randomMove_others(){
    for ( Hero & hero : heroes ){
        QDesktopWidget dw;
        if(hero.name!="Glados Entropy"){
            hero.randxy(dw.width(), dw.height(), ( h<w ) ?h/2:w/2);
